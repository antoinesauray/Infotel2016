...
public class HttpClientGet<T> extends AsyncTask<AbstractMap.SimpleEntry<String, String>, T, Integer> {
    ...
    private HttpClientGet(Context c, ProcessJson<T> processJson, Action<T> action, ErrorAction error, final String URL){
        this.c = c;
        this.action = action;
        this.error = error;
    ...
    public static AsyncTask getUberEstimate(Context c, Action<List<User>> action, ErrorAction error, double start_latitude, double start_longitude, double end_latitude, double end_longitude) {
        return new HttpClientGet<>(c, new ProcessJson<List<User>>() {
            @Override
